...
const STATUS_COLORS = { new: '#f59e0b', contacted: '#0891b2', followup: '#8b5cf6', converted: '#059669', closed: '#64748b' }
const STATUS_BG = { new: 'rgba(245,158,11,0.12)', contacted: 'rgba(8,145,178,0.12)', followup: 'rgba(139,92,246,0.12)', converted: 'rgba(5,150,105,0.12)', closed: 'rgba(100,116,139,0.12)' }
...
function LoginScreen({ onLogin }) {
  ...
  return (
    <div className="min-h-screen flex items-center justify-center" style={{ background: '#0f172a' }}>
      <div className="w-full max-w-sm p-8 rounded-2xl" style={{ background: 'rgba(30,41,59,0.9)', border: '1px solid rgba(8,145,178,0.2)' }}>
        <div className="text-center mb-8">
          <div className="font-display font-black text-4xl mb-1" style={{ color: '#0891b2' }}>VRM</div>
        ...
        <form onSubmit={submit} className="space-y-4">
          {['username','password'].map(f => (
            <div key={f}>
              ...
              <input type={f==='password'?'password':'text'} required className="w-full px-4 py-3 rounded-xl text-sm outline-none"
                style={{ background:'rgba(15,23,42,0.8)',border:'1px solid rgba(8,145,178,0.2)',color:'#e2e8f0' }}
          ...
          <button type="submit" disabled={loading} className="w-full py-3 rounded-xl font-bold text-sm"
            style={{ background:'linear-gradient(135deg,#0891b2,#0369a1)',color:'#ffffff' }}>
...
function StatCard({ label, value, color='#0891b2', icon }) {
  return (
...
function EnquiryRow({ enquiry, onStatusChange }) {
  ...
  return (
    <>
      <tr className="transition-colors cursor-pointer hover:bg-white/5" style={{ borderBottom:'1px solid rgba(8,145,178,0.08)' }} onClick={() => setExpanded(!expanded)}>
        <td className="px-4 py-4 font-bold font-display text-sm" style={{ color:'#0891b2' }}>{enquiry.enquiryId}</td>
        ...
        <td className="px-4 py-4 text-sm font-medium" style={{ color:'#0891b2' }}>{enquiry.phone}</td>
        ...
        <td className="px-4 py-4" onClick={e=>e.stopPropagation()}>
          <select disabled={updating} onChange={e=>changeStatus(e.target.value)} value={enquiry.status} className="text-xs px-2 py-1.5 rounded-lg outline-none"
            style={{ background:'rgba(15,23,42,0.8)',border:'1px solid rgba(8,145,178,0.2)',color:'#e2e8f0',cursor:'pointer' }}>
            {STATUS_OPTIONS.map(s=><option key={s} value={s}>{STATUS_ICONS[s]} {s.charAt(0).toUpperCase()+s.slice(1)}</option>)}
          </select>
        </td>
      </tr>
      {expanded && (
        <tr style={{ background:'rgba(8,145,178,0.03)',borderBottom:'1px solid rgba(8,145,178,0.08)' }}>
            ...
            <div className="grid sm:grid-cols-3 gap-4 text-xs mb-4">
              ...
              <div><div className="font-bold mb-1 uppercase tracking-widest" style={{ color:'#94a3b8' }}>Admin Notes</div>
                <textarea value={notes} onChange={e=>setNotes(e.target.value)} rows={2} placeholder="Add notes..."
                  ...
                  style={{ background:'rgba(15,23,42,0.8)',border:'1px solid rgba(8,145,178,0.2)',color:'#e2e8f0' }}/>
                <button onClick={()=>changeStatus(enquiry.status)} className="mt-1 px-3 py-1 rounded-lg text-xs font-bold" style={{ background:'rgba(8,145,178,0.15)',color:'#0891b2' }}>Save</button>
              </div>
            </div>
            <div className="flex gap-2 flex-wrap">
              <a href={`tel:${enquiry.phone}`} className="px-4 py-2 rounded-lg text-xs font-bold" style={{ background:'linear-gradient(135deg,#0891b2,#0369a1)',color:'#ffffff',textDecoration:'none' }}>📞 Call Now</a>
              ...
              {enquiry.email&&<a href={`mailto:${enquiry.email}`} className="px-4 py-2 rounded-lg text-xs font-bold" style={{ background:'rgba(8,145,178,0.1)',border:'1px solid rgba(8,145,178,0.2)',color:'#0891b2',textDecoration:'none' }}>✉️ Email</a>}
            </div>
          </td>
        </tr>
      )}
...
export default function AdminPage() {
  ...
  return (
    <div className="min-h-screen" style={{ background:'#0f172a' }}>
      <div className="sticky top-0 z-50 px-6 h-16 flex items-center justify-between" style={{ background:'rgba(15,23,42,0.95)',backdropFilter:'blur(20px)',borderBottom:'1px solid rgba(8,145,178,0.15)' }}>
        <div className="font-display font-black text-2xl" style={{ color:'#0891b2' }}>VRM Admin</div>
      ...
      <div className="px-6 py-8 max-w-[1400px] mx-auto">
        {analytics && (
          <div className="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
            <StatCard label="Total Enquiries" value={analytics.totalEnquiries} color="#0891b2" icon="📋"/>
        ...
        <div className="rounded-2xl overflow-hidden" style={{ background:'rgba(30,41,59,0.6)',border:'1px solid rgba(8,145,178,0.15)' }}>
          <div className="flex items-center justify-between px-6 py-4 flex-wrap gap-3" style={{ borderBottom:'1px solid rgba(8,145,178,0.1)' }}>
            ...
            <div className="flex gap-2 flex-wrap">
              {['', ...STATUS_OPTIONS].map(s => (
                <button key={s} onClick={() => setFilterStatus(s)} className="px-3 py-1.5 rounded-lg text-xs font-bold transition-all"
                  style={{ background:filterStatus===s?(s?STATUS_BG[s]:'rgba(8,145,178,0.15)'):'transparent', color:filterStatus===s?(s?STATUS_COLORS[s]:'#0891b2'):'#94a3b8', border:`1px solid ${filterStatus===s?(s?STATUS_COLORS[s]+'40':'rgba(8,145,178,0.3)'):'rgba(8,145,178,0.1)'}` }}>
          ...
          : <div className="overflow-x-auto"><table className="w-full">
              <thead><tr style={{ borderBottom:'1px solid rgba(8,145,178,0.1)' }}>
